// ui file to view supplier quotations
...
class viewSupplierQuotations extends Component{
...
componentWillMount(){
  ...
  var data=IndiaHacksContract.get_quotations({from: ETHEREUM_CLIENT.eth.accounts[4]})
  //console.log(data)
  ...
  this.setState(
    {
      ...
      prices:String(data[1]).split(','),
      vendors:String(data[2]).split(',')
    }
  )
  ...
  _.each(this.state.items,(value, index)=> {
     TableRows.push(
       ...
       <td><span style={{marginLeft:35,fontSize:25}}>{this.state.items[index]}</span></td>
            <td><span style={{marginLeft:35,fontSize:25}}>{this.state.prices[index]}</span></td>
...
<center>      <table style={{marginTop:80}}>
        <thead>
           ...
           <th><span style={{marginLeft:30,fontSize:25}}> Number </span></th>
              ...
              <th><span style={{marginLeft:30,fontSize:25}}> Price (in INR) </span></th>
...
export default viewSupplierQuotations;
